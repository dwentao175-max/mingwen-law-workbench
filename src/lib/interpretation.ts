...
// 系统固定的"结构契约"：所有模板（内置与自建）发送给模型时都会被强制套用，用户无法更改，以保证渲染器永远拿到同一套结构。
export const REPORT_STRUCTURE_CONTRACT = `任务：阅读【待解读文档】，按下方固定结构产出解读，仅输出严格 JSON（无任何多余文字、无 markdown 围栏）。
...
【输出 JSON 结构】
{
  "标题": "《XX》解读",
  "速览": {
    "发布机关":"", "发布日期":"", "施行日期":"", "文号":"", "效力层级":"",
    "章条规模":"如 7章39条", "上位法依据":["..."], "官方全文链接":"或null",
...
export const REPORT_CHUNK_SIZE = 12_000;
...
export function buildChunkStructuredPrompt(template: Template, chunk: string, index: number, total: number): string {
  return [
    buildStructuredPrompt(template, chunk),
    '',
